...
function generate_ajax_loads_table(scroll_y = '', pagination = true){
    let page_length = pagination ? 10 : -1;
    let length_menu = pagination ? [ 10, 25, 50 ] : false
    loads_table = $('#loads-table').DataTable({
        language: {
        ...
        ajax: {
            url:'/trainings/api/loads/?format=datatables',
            data: function(data){
                console.log(data)
            }
        },
...
$(window).on('load', function (){
    ...
    let old_data = {};
    $('#loads-table').on('click', '.edit-load', function(e) {
    ...
    $('#loads-table').on('click', '.save-load', function(e) {
        let row = $(this).closest('tr')
    ...
    $('#loads-table').on('click', '.delete-load', function(e) {
        let row = $(this).closest('tr')

        let id = row.attr('data-load')
        let send_data = {}

        ajax_loads_action('DELETE', send_data, 'delete load', id).then(function (data) {
            console.log(data)
            loads_table.ajax.reload()
        })
    })
    $('#loads-table').on('click', '.cancel-load', function(e) {
        let row = $(this).closest('tr')
