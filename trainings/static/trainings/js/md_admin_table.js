var amd_table;
var is_select_amd = false;

function generate_ajax_amd_table(scroll_y = '', pagination = true){
    let page_length = pagination ? 10 : -1;
    let length_menu = pagination ? [ 10, 25, 50 ] : false
    amd_table = $('#admin-md-table').DataTable({
        language: {
        ...
        rowCallback: function( row, data ) {
            console.log(data)
            $(row).attr('data-md', data.id)
        },
        ajax: {
            url:'/trainings/api/amd/?format=datatables',
            data: function(data){
                console.log(data)
            }
        },
        columns: [
            ...
            {'data': 'id', sortable: false, searchable: false, render: function (data, type, row, meta) {
                let button_html = ``
                if ($('#admin-md-table').attr('data-perm')=='True'){
                    button_html = `<div class="w-100 text-center" title="">`
                    button_html += `<button type="button" class="btn btn-sm btn-warning mx-1 swap-button edit-md py-0"><i class="fa fa-pencil" aria-hidden="true"></i></button>`
                    button_html += `<button type="button" class="btn btn-sm btn-danger mx-1 swap-button delete-md py-0"><i class="fa fa-trash" aria-hidden="true"></i></button>`
                    button_html += `<button type="button" class="btn btn-sm btn-success mx-1 swap-button save-md py-0 d-none"><i class="fa fa-floppy-o" aria-hidden="true"></i></button>`
                    button_html += `<button type="button" class="btn btn-sm btn-warning mx-1 swap-button cancel-md py-0 d-none"><i class="fa fa-ban" aria-hidden="true"></i></button>`
                    button_html += `</div>`
                }
                return button_html;
            }},
    ...
    $('#md-tab-button').on('shown.bs.tab', function () {
        amd_table.columns.adjust()
    })
    $('#nav-admin-md-tab').on('shown.bs.tab', function () {
        amd_table.columns.adjust()
    })
    $('.admin-md-table-filter').on("keyup change", function () {
        let val = $(this).val() ? $(this).val() : '';
        amd_table.columns($(this).attr('name')).search(val).draw();
    })
}

$(window).on('load', function (){
    // $('#admin-md-table').on('draw.dt', function () {
    //     console.log('draw')
    //     create_ajax_select2($('#admin-md-table .select2'), gettext('Short key'), '/trainings/amd_short', $('#references-modal'))
    //     $('#admin-md-table select').each(function (index) {
    ...
    // });
    let old_data = {};
    $('#admin-md-table').on('click', '.edit-md', function(e) {
    ...
    $('#admin-md-table').on('click', '.save-md', function(e) {
        let row = $(this).closest('tr')

        let id = row.attr('data-md')
        let send_data = {}
        send_data['name'] = row.find('[name="name"]').val()
        //send_data['short_name'] = row.find('[name="short_name"]').val()

        ajax_amd_action('PUT', send_data, 'edit md', id).then(function (data) {
            console.log(data)
            old_data = {}
            set_row_edit_mode(row, false)
        })
    })
    $('#admin-md-table').on('click', '.delete-md', function(e) {
        let row = $(this).closest('tr')

        let id = row.attr('data-md')
        let send_data = {}

        ajax_amd_action('DELETE', send_data, 'delete md', id).then(function (data) {
            console.log(data)
            amd_table.ajax.reload()
        })
    })
    $('#admin-md-table').on('click', '.cancel-md', function(e) {
        let row = $(this).closest('tr')
