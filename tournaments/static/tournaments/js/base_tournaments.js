    ...
    $.ajax({
        ...
        success: function (res) {
            if (res.success) {
                ...
                swal("Готово", "Турнир успешно создан / изменен.", "success");
                $('.row-content').removeClass('show-article-editor');
            } else {
                swal("Ошибка", `При создании / изменении турнира произошла ошибка (${res.err}).`, "error");
            }
        },
        error: function (res) {
            swal("Ошибка", "Турнир не удалось создать / изменить.", "error");
            console.log(res);
        },
        complete: function (res) {
            $('.page-loader-wrapper').fadeOut();
...
function DeleteArticle() {
    ...
    $.ajax({
        ...
        success: function (res) {
            if (res.success) {
                swal("Готово", "Турнир успешно удален.", "success");
                $('#deleteArticleModal').modal('hide');
            } else {
                swal("Ошибка", `При удалении турнира произошла ошибка (${res.err}).`, "error");
            }
        },
        error: function (res) {
            swal("Ошибка", "Турнир не удалось удалить.", "error");
            console.log(res);
        },
        complete: function (res) {
            $('.page-loader-wrapper').fadeOut();
...
function RenderSplitCols() {
    ...
    window.split = Split(['#splitCol_0', '#splitCol_1'], {
        ...
        onDragEnd: (arr) => {
            ...
            localStorage.setItem('split_cols__tournaments', JSON.stringify(window.dataForSplit));
        }
    });
...
$(function() {
    ...
    $('.article-editor-col').on('click', '.btn-save', (e) => {
        ...
        if (cTitle == "") {
            swal("Внимание", "Укажите название турнира.", "info");
            return;
        }
        if (cFolder == "") {
            swal("Внимание", "Выберите папку для турнира.", "info");
            return;
        }
        SaveArticle();
    });
    ...
    // Split columns
    window.dataForSplit = JSON.parse(localStorage.getItem('split_cols__tournaments'));
