...
class UserFolder(AbstractFolder):
    user = models.ForeignKey(User, on_delete=models.CASCADE, related_name='tournaments_userfolder_user')
    objects = models.Manager()
...
class ClubFolder(AbstractFolder):
    club = models.ForeignKey(Club, on_delete=models.CASCADE, null=True, blank=True, related_name='tournaments_clubfolder_club')
    objects = models.Manager()
...
class AbstractArticle(models.Model):
    ...
    visible = models.BooleanField(
        help_text='Показывать турнир пользователю или нет',
        default=True
    )
    completed = models.BooleanField(
        help_text='Турнир завершена',
        default=False
    )
    drafted = models.BooleanField(
        help_text='Турнир в черновике',
        default=False
    )
...
class UserArticle(AbstractArticle):
    user = models.ForeignKey(User, on_delete=models.CASCADE, related_name='tournaments_userarticle_user')
...
class ClubArticle(AbstractArticle):
    user = models.ForeignKey(User, on_delete=models.CASCADE, null=True, blank=True, related_name='tournaments_clubarticle_user')
    club = models.ForeignKey(Club, on_delete=models.CASCADE, null=True, blank=True, related_name='tournaments_clubarticle_club')
...
class UserArticleParam(models.Model):
    ...
    user = models.ForeignKey(User, on_delete=models.SET_NULL, null=True, related_name='tournaments_userarticleparam_user')
