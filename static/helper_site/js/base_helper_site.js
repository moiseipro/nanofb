...
function RenderSplitCols() {
    ...
    window.split = Split(['#splitCol_0', '#splitCol_1'], {
        ...
        onDragEnd: (arr) => {
            ...
            localStorage.setItem('split_cols__helper_site', JSON.stringify(window.dataForSplit));
        }
    });
...
$(function() {
    ...
    try {
        document.articleEditor = CKEDITOR.replace('articleEditor', {
            ...
            height: '61vh',
            filebrowserBrowseUrl: '/helper_site/ckeditorbrowse/',
            filebrowserImageBrowseUrl: '/helper_site/ckeditorbrowse/',
            filebrowserUploadUrl: '/helper_site/ckeditorupload/',
            filebrowserImageUploadUrl: '/helper_site/ckeditorupload/',
        });
            ...
            $('iframe.cke_wysiwyg_frame').ready((e) => {
                ...
                $(document).find('iframe.cke_wysiwyg_frame:first').contents().find('body').find('a').each((ind, elem) => {
                    ...
                    if ($(elem).hasClass('_doc_')) {
                        ...
                        $(elem).after(`
                            ${nfbVideoId != null ? `
                                <video id="video-player-helper_site-${ind}" class="video-js resize-block video-modal" poster="http://213.108.4.28/video/poster/${nfbVideoId}">
                                    <source src="${href}" type="video/mp4" />
                                </video>
                            ` : `
                                <video id="video-player-helper_site-${ind}" class="video-js resize-block video-modal" poster="">
                                    <source src="${href}" type="video/youtube" />
                                </video>
                            `}
                        `);
                        window.videoPlayerhelperSite.push(videojs(
                            $(document).find('iframe.cke_wysiwyg_frame:first').contents().find('body').find(`#video-player-helper_site-${ind}`)[0], {
                            preload: 'auto',
                            autoplay: false,
                            controls: true,
                            aspectRatio: '16:9',
                ...
                for (let i = 0; i < window.videoPlayerhelperSite.length; i++) {
                    window.videoPlayerhelperSite[i].load();
                    window.videoPlayerhelperSite[i].ready((e) => {
                        $(document).find('iframe.cke_wysiwyg_frame:first').contents().find('body').find(`#video-player-helper_site-${i}`)
                            .css('height', '88vh');
                    });
                }
            });
    ...
    // Split columns
    window.dataForSplit = JSON.parse(localStorage.getItem('split_cols__helper_site'));
