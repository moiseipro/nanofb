...
function RenderSplitCols() {
    ...
    window.split = Split(['#splitCol_0', '#splitCol_1'], {
        ...
        onDragEnd: (arr) => {
            ...
            localStorage.setItem('split_cols__helper_football', JSON.stringify(window.dataForSplit));
        }
    });
    ...
    .then(data => {
        if (data) {
            if ('nftv' in data['links'] && data['links']['nftv'] != '') {
                $('#videoSelectorModal').find('input.video-link').val(`http://213.108.4.28/video/player/${data['links']['nftv']}`);
                windowElem.src({type: 'video/mp4', src: `http://213.108.4.28/video/player/${data['links']['nftv']}`});
                windowElem.poster(`http://213.108.4.28/video/poster/${data['links']['nftv']}`);
...
$(function() {
    ...
    try {
        document.articleEditor = CKEDITOR.replace('articleEditor', {
            ...
            height: '61vh',
            filebrowserBrowseUrl: '/helper_football/ckeditorbrowse/',
            filebrowserImageBrowseUrl: '/helper_football/ckeditorbrowse/',
            filebrowserUploadUrl: '/helper_football/ckeditorupload/',
            filebrowserImageUploadUrl: '/helper_football/ckeditorupload/',
        });
    ...
    try {
        ...
        document.articleViewer.on('change', (evt) => {
            $('iframe.cke_wysiwyg_frame').ready((e) => {
                ...
                window.videoPlayerhelperFootball = [];
                $(document).find('iframe.cke_wysiwyg_frame:first').contents().find('body').find('a').each((ind, elem) => {
                    ...
                    if ($(elem).hasClass('_doc_')) {
                        ...
                        $(elem).after(`
                            ${nfbVideoId != null ? `
                                <video id="video-player-helper_football-${ind}" class="video-js resize-block video-modal" poster="http://213.108.4.28/video/poster/${nfbVideoId}">
                                    <source src="${href}" type="video/mp4" />
                                </video>
                            ` : `
                                <video id="video-player-helper_football-${ind}" class="video-js resize-block video-modal" poster="">
                                    <source src="${href}" type="video/youtube" />
                                </video>
                            `}
                        `);
                        window.videoPlayerhelperFootball.push(videojs(
                            $(document).find('iframe.cke_wysiwyg_frame:first').contents().find('body').find(`#video-player-helper_football-${ind}`)[0], {
                            preload: 'auto',
                            autoplay: false,
                            controls: true,
                            aspectRatio: '16:9',
                ...
                for (let i = 0; i < window.videoPlayerhelperFootball.length; i++) {
                    window.videoPlayerhelperFootball[i].load();
                    window.videoPlayerhelperFootball[i].ready((e) => {
                        $(document).find('iframe.cke_wysiwyg_frame:first').contents().find('body').find(`#video-player-helper_football-${i}`)
                            .css('height', '88vh');
                    });
                }
            });
    ...
    // Split columns
    window.dataForSplit = JSON.parse(localStorage.getItem('split_cols__helper_football'));
