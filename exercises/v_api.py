...
def get_excerises_data(folder_id=-1, folder_type="", req=None, cur_user=None, cur_team=None, to_count=False, count_for_tag=None, exercise_id=-1, tags_folder=False, tags_short_categories=False):
    ...
    try:
        if req.method == "GET":
            filter_note_status = int(req.GET.get("filter[note_status]", -1))
        elif req.method == "POST":
    ...
    try:
        if req.method == "GET":
            filter_language_complete = int(req.GET.get("filter[language_complete]", -1))
        elif req.method == "POST":
            filter_language_complete = int(req.POST.get("filter[language_complete]", -1))
    except:
        pass
    f_exercises = []
    c_folder = None
    ...
    if len(filter_tags_folder) > 0:
        ...
        for f_tag in filter_tags_folder:
            q_objects |= Q(tags_folder__lowercase_name__icontains=f_tag)
    ...
    if filter_note_status != -1:
        if req.user.club_id is not None:
            f_exercises = f_exercises.filter(
                Q(Q(userexerciseparam__note_status__gt=0), userexerciseparam__user=None)
            ).distinct()
        else:
            f_exercises = f_exercises.filter(
    ...
    if filter_language_complete != -1:
        if filter_language_complete != 2:
            f_exercises = f_exercises.filter(
                Q(**{f'languages_complete__{key}': f'{filter_language_complete}' for key in f_exercises.values_list('languages_complete', flat=True)})
            ).distinct()
        else:
            f_exercises = f_exercises.exclude(
                Q(**{f'languages_complete__{key}': '0' for key in f_exercises.values_list('languages_complete', flat=True)}) |
    ...
    if to_count:
        ...
        for exercise in f_exercises_list:
            ...
            if user_params != None and user_params.exists() and user_params[0].id != None:
                ...
                exercise['note_trainer'] = user_params['note_trainer']
                exercise['note_club_admin'] = user_params['note_club_admin']
                exercise['note_status'] = user_params['note_status']
            if user_params_club != None and user_params_club.exists() and user_params_club[0].id != None:
                ...
                exercise['note_status'] = user_params_club['note_status']
            watched_status = 0
...
def POST_edit_exs(request, cur_user, cur_team):
    ...
    if is_can_edit_full:
        ...
        c_exs.tags_folder.clear()
        tags_arr = utils.set_value_as_list(request, "data[tags_folder]", "data[tags_folder][]", [])
        for c_tag in tags_arr:
            c_tag_lower = c_tag.lower()
            f_tag = None
            try:
                if folder_type == utils.FOLDER_TEAM and request.user.club_id is None:
                    f_tag = ExerciseTagFolder.objects.filter(
                        Q(is_nfb=True, lowercase_name=c_tag_lower) | Q(is_nfb=False, user=cur_user, lowercase_name=c_tag_lower)
                    )
                    if f_tag.exists() and f_tag[0].id != None:
                        f_tag = f_tag[0]
                    else:
                        f_tag = ExerciseTagFolder(is_nfb=False, user=cur_user, name=c_tag, lowercase_name=c_tag_lower)
                        f_tag.save()
                elif folder_type == utils.FOLDER_NFB:
                    f_tag = ExerciseTagFolder.objects.filter(is_nfb=True, lowercase_name=c_tag_lower)
                    if f_tag.exists() and f_tag[0].id != None:
                        f_tag = f_tag[0]
                    else:
                        f_tag = ExerciseTagFolder(is_nfb=True, name=c_tag, lowercase_name=c_tag_lower)
                        f_tag.save()
                elif folder_type == utils.FOLDER_CLUB or folder_type == utils.FOLDER_TEAM and request.user.club_id is not None:
                    f_tag = ExerciseTagFolder.objects.filter(
                        Q(is_nfb=True, lowercase_name=c_tag_lower) | Q(is_nfb=False, club=request.user.club_id, lowercase_name=c_tag_lower)
                    ...
                    if f_tag.exists() and f_tag[0].id != None:
                        ...
                        f_tag = ExerciseTagFolder(is_nfb=False, club=request.user.club_id, name=c_tag, lowercase_name=c_tag_lower)
                        f_tag.save()
            except:
                pass
            if f_tag is not None:
...
def POST_edit_exs_user_params(request, cur_user, cur_team):
    ...
    try:
        ...
        pass
    if post_key == "note_trainer" or post_key == "note_club_admin":
        post_value = ""
        try:
...
def POST_edit_exs_tag_folder_one(request, cur_user):
    status = False
    c_type = request.POST.get("type", "")
    c_id = -1
    delete_status = -1
    ...
    if c_type == "nfb":
        if cur_user.is_superuser:
            c_tag = ExerciseTagFolder.objects.filter(id=c_id, is_nfb=True).first()
            if delete_status != 1:
                if c_tag:
                    c_tag.name = c_name
                    c_tag.lowercase_name = lowercase_name
                    ...
                    c_tag = ExerciseTagFolder(is_nfb=True, name=c_name, lowercase_name=lowercase_name, short_name=c_short_name)
                try:
                    c_tag.save()
                    status = True
                except Exception as e:
        ...
        c_tag = None
        if request.user.club_id is not None:
            c_tag = ExerciseTagFolder.objects.filter(id=c_id, is_nfb=False, club=request.user.club_id).first()
        else:
            c_tag = ExerciseTagFolder.objects.filter(id=c_id, is_nfb=False, user=cur_user).first()
        if delete_status != 1:
            if c_tag:
                c_tag.name = c_name
                c_tag.lowercase_name = lowercase_name
                ...
                if request.user.club_id is not None:
                    c_tag = ExerciseTagFolder(is_nfb=False, club=request.user.club_id, name=c_name, lowercase_name=lowercase_name, short_name=c_short_name)
                else:
                    c_tag = ExerciseTagFolder(is_nfb=False, user=cur_user, name=c_name, lowercase_name=lowercase_name, short_name=c_short_name)
            try:
                c_tag.save()
                status = True
            except Exception as e:
...
def POST_change_order_exs_tag_folder_one(request, cur_user):
    status = True
    ids_data = request.POST.getlist("ids_arr[]", [])
    ordering_data = request.POST.getlist("order_arr[]", [])
    c_type = request.POST.get("type", "")
    ...
    for c_ind in range(len(ids_data)):
        ...
        if c_type == "nfb":
            if cur_user.is_superuser:
                found_param = ExerciseTagFolder.objects.filter(id=t_id, is_nfb=True).first()
        elif c_type == "self":
            if request.user.club_id is not None:
...
def GET_get_exs_all(request, cur_user, cur_team):
    ...
    for exercise in found_exercises:
        ...
        exs_data = {
            ...
            'opt_has_description_2': exercise['opt_has_description_2'],
            'opt_has_scheme': exercise['opt_has_scheme'],
            'opt_has_scheme_2': exercise['opt_has_scheme_2'],
        ...
        exs_data['note_status'] = exercise['note_status'] if 'note_status' in exercise else None
...
def GET_get_exs_one(request, cur_user, cur_team, additional={}):
    ...
    if res_exs['scheme_1_old'] is not None:
        try:
            ...
            if 'id' in r_json:
                new_scheme_id = r_json['id']
                exs_to_save = c_exs.first()
                exs_to_save.scheme_1 = new_scheme_id
                exs_to_save.scheme_1_old = None
                exs_to_save.save(update_fields=["scheme_1", "scheme_1_old"])
    ...
    if res_exs['scheme_2_old'] is not None:
        try:
            ...
            if 'id' in r_json:
                new_scheme_id = r_json['id']
                exs_to_save = c_exs.first()
                exs_to_save.scheme_2 = new_scheme_id
                exs_to_save.scheme_2_old = None
                exs_to_save.save(update_fields=["scheme_2", "scheme_2_old"])
...
def GET_get_exs_all_tags(request, cur_user, cur_team, tags_folder=False, tags_short_categories=False):
    ...
    for entry in tags['nfb']:
        data['nfb'].append({
            ...
            'short_name': entry.short_name if hasattr(entry, 'short_name') and getattr(entry, 'short_name') is not None else "",
            'category': entry.category.id if hasattr(entry, 'category') and getattr(entry, 'category') is not None else "",
...
def GET_check_copied_nf_exs(request, cur_user, cur_team):
    ...
    for exs_id in exs_ids:
        ...
        if request.user.club_id is not None:
            ...
            c_exs = UserExercise.objects.filter(clone_nfb_id=t_id, visible=True, user=cur_user, folder__team=cur_team).first()
        if c_exs:
